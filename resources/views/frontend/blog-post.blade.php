<x-layouts.frontend :title="($post->meta_title ?: $post->title) . ' — Redis Solution Blog'">
    ...
    <section class="photo-hero">
        ...
        <div class="container photo-hero__content">
            ...
            <div style="display:flex;align-items:center;gap:1.5rem;flex-wrap:wrap;margin-top:0.75rem">
                <span style="font-size:0.85rem;color:rgba(255,255,255,0.55)"><i class="ri-user-line"></i> {{ $post->author->name ?? 'Redis Solution' }}</span>
                <span style="font-size:0.85rem;color:rgba(255,255,255,0.55)"><i class="ri-calendar-line"></i> {{ $post->published_at->format('d M Y') }}</span>
                <span style="font-size:0.85rem;color:rgba(255,255,255,0.55)"><i class="ri-time-line"></i> {{ $post->reading_time }} min read</span>
                <span style="font-size:0.85rem;color:rgba(255,255,255,0.55)"><i class="ri-eye-line"></i> {{ number_format($post->views_count) }} views</span>
            </div>
        </div>
    </section>
    ...
    <section class="section">
        <div class="container">
            <div style="display:grid;grid-template-columns:1fr 260px;gap:3rem;align-items:start">
                ...
                <article>
                    @if($post->excerpt)
                        <p style="font-size:1.1rem;color:var(--fg-text-muted);line-height:1.75;border-left:3px solid #FF6400;padding-left:1.25rem;margin-bottom:2.5rem;font-style:italic">
                            {{ $post->excerpt }}
                        </p>
                    @endif
                    ...
                    {{-- Share --}}
                    <div style="margin-top:3rem;padding-top:2rem;border-top:1px solid var(--fg-border);display:flex;align-items:center;gap:1rem;flex-wrap:wrap">
                        <span style="font-size:0.875rem;font-weight:600;color:var(--fg-text-muted)">Share:</span>
                ...
                <aside style="position:sticky;top:7rem;display:flex;flex-direction:column;gap:1.5rem">
                    ...
                    {{-- Author card --}}
                    <div style="background:var(--fg-card);border:1px solid var(--fg-card-border);border-radius:14px;padding:1.25rem;text-align:center">
                        ...
                        <div style="font-weight:700;color:var(--fg-heading);font-size:0.9rem">{{ $post->author->name ?? 'Redis Solution' }}</div>
                        <div style="font-size:0.78rem;color:var(--fg-text-muted);margin-top:0.2rem">Redis Solution Team</div>
                    </div>

                    {{-- CTA --}}
                    <div style="background:linear-gradient(135deg,rgba(255,100,0,0.1),rgba(236,72,153,0.06));border:1px solid rgba(255,100,0,0.2);border-radius:14px;padding:1.5rem;text-align:center">
                        <p style="font-family:'Syne',sans-serif;font-size:0.95rem;font-weight:700;color:var(--fg-heading);margin:0 0 0.5rem">Ready to start?</p>
                        <p style="font-size:0.8rem;color:var(--fg-text-muted);margin:0 0 1.25rem;line-height:1.5">Talk to our team about your project — free consultation.</p>
                    ...
                    <a href="{{ route('blog.index') }}"
                        style="display:flex;align-items:center;gap:0.5rem;font-size:0.85rem;color:var(--fg-text-muted);text-decoration:none;padding:0.75rem 0;border-top:1px solid var(--fg-border);transition:color 0.2s"
                        onmouseover="this.style.color='#FF6400'" onmouseout="this.style.color='var(--fg-text-muted)'">
                    ...
                    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1.5rem">
                        @foreach($related as $rel)
                            <a href="{{ route('blog.show', $rel->slug) }}"
                                style="display:block;background:var(--fg-card);border:1px solid var(--fg-card-border);border-radius:12px;overflow:hidden;text-decoration:none;transition:border-color 0.2s"
                                onmouseover="this.style.borderColor='rgba(255,100,0,0.3)'" onmouseout="this.style.borderColor='var(--fg-card-border)'">
                                @if($rel->featured_image)
                                    <img src="{{ $rel->featured_image }}" alt="{{ $rel->title }}" style="width:100%;height:130px;object-fit:cover">
                                @endif
    ...
    <style>
        ...
        .blog-content ul,
        .blog-content ol { padding-left: 1.5rem; margin: 0 0 1.25rem; }
        .blog-content li { margin-bottom: 0.4rem; }
        .blog-content a { color: #FF6400; text-decoration: underline; }
        .blog-content strong { color: var(--fg-heading); font-weight: 700; }
        .blog-content em { color: var(--fg-text-muted); }
        ...
        /* Code blocks always dark — better readability */
        .blog-content pre {
            background: #0d1117;
