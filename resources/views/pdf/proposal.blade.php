...
@php
    ...
    // Delivery Days column — only show if at least one item has a value
    $hasDeliveryDays = $proposal->items->filter(fn($i) => !empty($i->delivery_days))->isNotEmpty();
    $showPrices = $proposal->isSectionEnabled('prices');
@endphp

<style>
@page { margin: 0 0 32px 0; size: A4 portrait; }
        ...
        <tr>
            ...
            <td style="width:45%;vertical-align:top;text-align:right">
                ...
                <div style="font-size:7.5pt;color:#555;line-height:2;margin-top:7px">
                    ...
                    @if($validUntilStr)Valid Until: {{ $validUntilStr }}<br>@endif
...
@if($proposal->isSectionEnabled('scope') && $proposal->items->count())
    <div style="padding:14px 28px;border-bottom:1px solid #ebebeb">
        ...
        <table class="itbl">
            <colgroup>
                <col style="width:4%">
                @if($hasDeliveryDays && $showPrices)
                    <col style="width:47%">
                    <col style="width:14%">
                    <col style="width:35%">
                @elseif($hasDeliveryDays)
                    <col style="width:82%">
                    <col style="width:14%">
                @elseif($showPrices)
                    <col style="width:61%">
            ...
            <tbody>
                @foreach($proposal->items as $idx => $item)
                    <tr class="{{ $idx % 2 === 1 ? 'itbl-even' : '' }}" style="page-break-inside:avoid">
                        ...
                        @if($hasDeliveryDays)
                            <td style="text-align:center;font-size:8pt;color:#555;white-space:nowrap">{{ $item->delivery_days ?: '—' }}</td>
                        @endif
                        @if($showPrices)
...
@if($proposal->isSectionEnabled('pricing'))
    <div style="padding:14px 28px;border-bottom:1px solid #ebebeb;page-break-inside:avoid">
        ...
        @if($proposal->milestone_mode && !empty($proposal->milestones))
            ...
            <table style="width:100%;border-collapse:collapse;margin-bottom:8px">
                <thead>
                    <tr style="background:#f5f5f5">
                        <th style="padding:6px 10px;font-size:7pt;font-weight:bold;text-transform:uppercase;letter-spacing:0.05em;color:#555;text-align:left">Payment Milestone</th>
                        <th style="padding:6px 10px;font-size:7pt;font-weight:bold;text-transform:uppercase;letter-spacing:0.05em;color:#555;text-align:right;width:140px">Amount</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($proposal->milestones as $ms)
                        <tr style="border-bottom:1px solid #eee;page-break-inside:avoid">
                            <td style="padding:7px 10px;font-size:8.5pt;color:#333">{{ $ms['label'] ?? '' }}</td>
                            <td style="padding:7px 10px;font-size:8.5pt;font-weight:bold;color:#FF6400;text-align:right;white-space:nowrap">
            ...
            <table style="width:100%;border-collapse:collapse;border:2px solid #FF6400">
                <tr>
                    <td style="font-size:10pt;font-weight:bold;color:#FF6400;padding:7px 10px;text-align:left;letter-spacing:1px">TOTAL</td>
            ...
            <table style="width:100%;border-collapse:collapse">
                <tr>
                    ...
                    <td style="width:45%;vertical-align:top">
                        <table style="width:100%;border-collapse:collapse;border:2px solid #FF6400">
                            <tr>
                                <td style="font-size:10pt;font-weight:bold;color:#FF6400;padding:9px 12px;text-align:left;letter-spacing:1px">TOTAL</td>
                                <td style="font-size:10pt;font-weight:bold;color:#FF6400;padding:9px 12px;text-align:right;white-space:nowrap">{{ $proposal->currency }}&nbsp;{{ number_format((float) $proposal->total_amount, 2) }}</td>
                            </tr>
                        </table>
                    </td>
                </tr>
...
{{-- TAGLINE --}}
<div style="text-align:center;padding:10px 28px 8px;font-size:7pt;color:#aaa;text-transform:uppercase;letter-spacing:0.15em;page-break-inside:avoid">
