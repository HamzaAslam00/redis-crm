<x-layouts.backend title="Edit Note">
    ...
    <div style="display:flex;align-items:center;gap:1rem;margin-bottom:2rem">
        ...
        <div>
            ...
            <p class="page-subtitle">{{ $note->title ?: 'Untitled Note' }}</p>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('admin.notes.update', $note) }}">
            ...
            <div class="form-group" style="margin-bottom:1.25rem">
                ...
                <input type="text" id="title" name="title"
                    ...
                    value="{{ old('title', $note->title) }}" placeholder="Optional title…" autofocus>
            ...
            <div class="form-group" style="margin-bottom:1.25rem">
                ...
                <textarea id="content" name="content" rows="12"
                    class="form-control @error('content') is-invalid @enderror">{{ old('content', $note->content) }}</textarea>
            ...
            <div class="form-grid-2" style="margin-bottom:1.25rem">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="tags_input" name="tags_input"
                        ...
                        value="{{ old('tags_input', $note->tags ? implode(', ', $note->tags) : '') }}"
                        placeholder="Comma-separated: work, idea, todo">
                    ...
                    <div style="display:flex;flex-wrap:wrap;gap:0.5rem;margin-top:0.35rem">
                        @foreach($colors as $hex => $name)
                            <label title="{{ $name }}" style="cursor:pointer">
                                <input type="radio" name="color" value="{{ $hex }}" {{ old('color', $note->color ?? '#ffffff') === $hex ? 'checked' : '' }} style="display:none">
                                <span style="display:block;width:28px;height:28px;border-radius:50%;background:{{ $hex }};border:2px solid {{ old('color', $note->color ?? '#ffffff') === $hex ? '#FF6400' : 'rgba(0,0,0,0.15)' }};transition:border-color 0.15s"></span>
                            </label>
                        @endforeach
                    </div>
                </div>
            ...
            <div style="display:flex;align-items:center;justify-content:flex-end;gap:0.75rem;padding-top:1rem;border-top:1px solid var(--crm-border);margin-top:0.5rem">
                ...
                <button type="submit" class="btn btn-primary"><i class="ri-save-line"></i> Update Note</button>
            </div>
        </form>
    </div>
