<x-layouts.backend title="Edit API Key">
    ...
    <div style="display:flex;align-items:center;gap:1rem;margin-bottom:2rem">
        ...
        <div>
            ...
            <p class="page-subtitle">{{ $apiKey->provider_name }} — {{ $apiKey->key_label }}</p>
        </div>
    </div>

    <div class="crm-card">
        <form method="POST" action="{{ route('admin.api-keys.update', $apiKey) }}">
            @csrf
            @method('PUT')
            <div class="form-grid-2">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="provider_name" name="provider_name"
                        ...
                        value="{{ old('provider_name', $apiKey->provider_name) }}" autofocus>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="key_label" name="key_label"
                        ...
                        value="{{ old('key_label', $apiKey->key_label) }}">
                ...
                <div class="form-group" style="grid-column:span 2">
                    ...
                    <p style="font-size:0.75rem;color:var(--crm-text-muted);margin-top:0.35rem"><i class="ri-information-line"></i> Leave blank to keep the existing encrypted key.</p>
                ...
                <div class="form-group">
                    ...
                    <select id="key_type" name="key_type" class="form-control @error('key_type') is-invalid @enderror">
                        @foreach($keyTypes as $k => $label)
                            <option value="{{ $k }}" {{ old('key_type', $apiKey->key_type) === $k ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select id="environment" name="environment" class="form-control @error('environment') is-invalid @enderror">
                        @foreach($environments as $k => $label)
                            <option value="{{ $k }}" {{ old('environment', $apiKey->environment) === $k ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group" style="grid-column:span 2">
                    ...
                    <textarea id="notes" name="notes" rows="3"
                        class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $apiKey->notes) }}</textarea>
            ...
            @if($apiKey->last_used_at)
                <p style="font-size:0.75rem;color:var(--crm-text-muted);margin-bottom:1rem">
                    <i class="ri-time-line"></i> Last revealed: {{ $apiKey->last_used_at->diffForHumans() }}
                </p>
            @endif
            ...
            <div style="display:flex;align-items:center;justify-content:flex-end;gap:0.75rem;padding-top:1rem;border-top:1px solid var(--crm-border);margin-top:0.5rem">
                ...
                <button type="submit" class="btn btn-primary"><i class="ri-save-line"></i> Update Key</button>
            </div>
        </form>
    </div>
