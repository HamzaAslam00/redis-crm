<x-layouts.backend title="Add Expense">
        ...
        <form method="POST" action="{{ route('admin.budget.expenses.store') }}">
            ...
            <div class="form-grid-2">
                ...
                <div class="form-group" style="grid-column:span 2">
                    <label class="form-label" for="reason">Reason <span style="color:#F87171">*</span></label>
                    <input type="text" id="reason" name="reason"
                        class="form-control @error('reason') is-invalid @enderror"
                        value="{{ old('reason') }}" placeholder="What was this expense for?" autofocus>
                    @error('reason')<p class="form-error">{{ $message }}</p>@enderror
                </div>

                <div class="form-group">
                    <label class="form-label" for="type">Type <span style="color:#F87171">*</span></label>
                    <select id="type" name="type" class="form-control @error('type') is-invalid @enderror">
                        @foreach($types as $key => $label)
                            <option value="{{ $key }}" {{ old('type', 'other') === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
            ...
            <div style="display:flex;align-items:center;justify-content:flex-end;gap:0.75rem;padding-top:1rem;border-top:1px solid var(--crm-border);margin-top:0.5rem">
                ...
                <button type="submit" class="btn btn-primary"><i class="ri-save-line"></i> Save Expense</button>
            </div>
        </form>
    </div>
