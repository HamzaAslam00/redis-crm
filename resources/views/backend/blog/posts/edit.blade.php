<x-layouts.backend title="Edit Post">

    <div style="display:flex;align-items:center;gap:1rem;margin-bottom:2rem">
        <a href="{{ route('admin.blog.posts.index') }}" class="btn btn-secondary btn-sm"><i class="ri-arrow-left-line"></i></a>
        <div>
            <h1 class="page-title">Edit Post</h1>
            <p class="page-subtitle">{{ Str::limit($post->title, 60) }}</p>
        </div>
    </div>

    {{-- Pass initial content to JS before Alpine boots --}}
    <script>window.__blogEditorInit = @json(old('content', $post->content));</script>
                ...
                <div class="crm-card">
                    <div class="form-group">
                        ...
                        <input type="text" id="title" name="title"
                            ...
                            value="{{ old('title', $post->title) }}" style="font-size:1.1rem;font-weight:600">
                    ...
                    <div class="form-group" style="margin-top:1rem">
                        ...
                        <textarea name="content" x-ref="contentInput" style="display:none">{{ old('content', $post->content) }}</textarea>
            ...
            <div style="display:flex;flex-direction:column;gap:1.5rem;position:sticky;top:1.5rem">
                ...
                <div class="crm-card">
                    ...
                    @if($post->published_at)
                        <p style="font-size:0.78rem;color:var(--crm-text-muted);margin-top:0.5rem">
                            <i class="ri-calendar-check-line"></i> Published {{ $post->published_at->format('d M Y') }}
                        </p>
                    @endif
                ...
                <div class="crm-card">
                    ...
                    @if($post->featured_image)
                        <img src="{{ $post->featured_image }}" alt="" style="width:100%;height:140px;object-fit:cover;border-radius:8px;margin-bottom:0.75rem">
                    @endif
                    <input type="text" name="featured_image"
                        class="form-control @error('featured_image') is-invalid @enderror"
                        value="{{ old('featured_image', $post->featured_image) }}" placeholder="https://… or /storage/…">
    ...
    <form id="delete-post-form" method="POST" action="{{ route('admin.blog.posts.destroy', $post) }}" style="display:none">
        @csrf
        @method('DELETE')
    </form>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('blogEditor', () => ({
        ...
        initEditor(html) {
            ...
            // Image click listener for position bar
            const self = this;
